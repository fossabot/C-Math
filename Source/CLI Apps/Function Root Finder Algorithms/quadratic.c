...
#include "../../Library/Function Root Finder Algorithms/quadraticAlgorithm.h"
...
int main() {
    ...
    char *fgetsReturn;
    char a_c[ASL_INPUT_SIZE], b_c[ASL_INPUT_SIZE], c_c[ASL_INPUT_SIZE], verbose_c[ASL_INPUT_SIZE],
    ...
    printf("Enter coefficients a, b, c of function F(X) = \"a*x^2 + bx + c\":\n");
    ...
    // calculation
    double *result = asl_quadratic_real_root(a, b, c, verbose, &flag);

    // if there was an answer
    if (flag == ASL_HAS_A_ROOT) {
        printf("\nThe function F(X) = %g * x^2 %+g * x %+g , has one root at point: x(1) = %g .\n", a, b, c, result[0]);
    } else if (flag == ASL_HAS_DOUBLE_ROOTS) { // if no answer
        printf("\nThe function F(X) = %g * x^2 %+g * x %+g , has double root at point: x(1, 2) = %g .\n",
               a, b, c, result[0]);
    } else if (flag == ASL_HAS_TWO_ROOTS) {
        printf("\nThe function F(X) = %g * x^2 %+g * x %+g , has two roots at points: x(1) = %g , x(2) = %g .\n",
               a, b, c, result[0], result[1]);
    } else {
        printf("\nThe function F(X) = %g * x^2 %+g * x %+g , doesn't have any roots .\n", a, b, c);
    } // end of if else
